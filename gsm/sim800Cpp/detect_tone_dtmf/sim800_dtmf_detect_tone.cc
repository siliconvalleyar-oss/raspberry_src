...
bool sendATCommand(int serial, const string& command, string& response) {
    ...
    while (true) {
        ...
        if (bytesRead > 0) {
            ...
            // Check if the response contains a newline, indicating the end of the line
            if (response.find("\r\n") != string::npos) {
                // Process the complete line here
                if (response.find("OK\r\n") != string::npos ||
                    response.find("ERROR") != string::npos ||
                    response.find(">") != string::npos ||
                    response.find("+DTMF:") != string::npos) {
                    return true;
                }
                response.clear(); // Clear the response buffer for the next line
            }
        } else if (bytesRead < 0) {
